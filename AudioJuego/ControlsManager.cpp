...
uint16_t ControlsManager::joyX = 512;
uint16_t ControlsManager::joyY = 512;
...
void ControlsManager::UpdateVolume(uint32_t deltaTime) {
    ...
    AudioManager::SetGlobalVolume(AudioManager::GetVolume() + (float)(volume) / 10.0f);
...
void ControlsManager::UpdateVibrations(uint32_t deltaTime) {
    ...
    if (vibrationMillisRight > 0) {
        ...
        if (static_cast<uint32_t>(vibrationMillisRight) <= deltaTime)
            vibrationMillisRight = 0;
        else
    ...
    if (vibrationMillisLeft > 0) {
        ...
        if (static_cast<uint32_t>(vibrationMillisLeft) <= deltaTime)
            vibrationMillisLeft = 0;
        else
